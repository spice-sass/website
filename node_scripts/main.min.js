...
$(document).ready(function(){
	...
	// converter helper - dont forget to copy changes to root!!!!!
	// #############################################################

	var $mixins = $('.main__item');

	$mixins.each(function(){


		var $this       = $(this),
			name        = $this.find('.item__heading').text(),
			desc        = $this.find('.item__description').html(),
			tableClass  = $this.find('.item__parameters'),
			args        = tableClass.find('tr').length - 1,
			params      = "<table>" + tableClass.html() + "</table>",
			include     = $this.find('.example__code.language-scss').parent(),
			mixin       = $this.find('.item__code.item__code--togglable.language-scss').parent(),
			output      = $this.find('.example__code.language-css').parent(),
			html        = $this.find('.example__code.language-markup').parent(),
			usedby      = $this.find('.list-unstyled li'),
			headings    = $this.find('.item__sub-heading'),
			demo        = $this.find('.example__code.language-markup').find('code');

		include.find('.example__description').remove();
		output.find('.example__description').remove();

		if(html){
			html.find('.example__description').remove();
		}
		

		mixin.click(function(){
			$(this).find('pre').attr('data-expanded','').attr('data-collapsed','');
		});


		$this.css({position:"relative"});

		var template = "<button style='position:absolute;top:80px;right:30px;' class='convertToJson'>Convert</button>";

		$this.append(template);

		var $converter = $this.find('.convertToJson');

		$converter.click(function(){

			var links,LinksTitle;

			if(headings.text().indexOf('Used by') > -1){

				var links = [];
				var LinksTitle = "Used by";

				usedby.each(function(){
					links.push($(this).find('a').text())
				});


			} else if (headings.text().indexOf('Requires') > -1){

				var links = [];
				var LinksTitle = "Requires";
			...
			var json = {
				...
				"args"       : args,
				"params"     : params,
				"include"    : include[0].innerHTML,
				"mixin"      : mixin[0].innerHTML,
				"output"     : output[0].innerHTML,
				"demo"       : (demo.length > 0)? demo.text() : false,
				"markup"     : (html.length > 0)? html[0].innerHTML : false,
				"links"      : links,
